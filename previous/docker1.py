...
class DockerGame:
    def __init__(self, options):
        ...
        self.reward_map = {
            ...
            'early_finish': 100.0,
            'zero_cover_num': 0.2,
            'replace_times_num': 0.2,
            'huge_car': 2.2,
            'min_car': 0.3,
            'cover_expire': 0.1,
            'long_day': 0.6,
            'mix_score': 0.6,
            'same_bill_lading': 0.4,
            'same_party': 0.4,
            'bind_container': 0.4,
            'book_container': 0.4,
            'used_container': 0.4,
            'check_container': 0.4,
            'block_container': 0.4,
            'relocation_times': 2.0,
            'twice_relocation_times': 0.2,
            'assign_container': 0.4,
            'leave_success': 8.0,
            'enter_success': 8.0
        }
    ...
    def take_action(self, input_pile=None):
        ...
        if len(self.operation_list) > 0:
            ...
            detail, enter_able_pile = self.next_container()
             # 打印调试信息
            # c_stack = f'{self.options.block}{utils.to_double(pile_place[0])}{utils.to_double(pile_place[1])}'
            # print("c_stack:", c_stack)
            # print("yard_stack_map keys sample:", list(self.yard_stack_map.keys())[:5])

            if score is None:
                score = 0
            print(f"action_step score = {score}")
