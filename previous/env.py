...
import previous.docker1 as docker
import previous.utils as utils
...
class DockerYard(gym.Env):
    ...
    def create_game(self):
        ...
        # print(">>action space size:",len(self.action_map))
    ...
    def step(self, action):
        ...
        if len(self.docker_game.operation_list) > 0:
            if (action[0] != 0 and action[1] != 0 and action[-1] != 0) or (
                ...
                if self.docker_game.options.with_predict:
                    ...
                    # print(">>>current_operation_type:", current_operation_type,
                    #       "current_container_size:", current_container_size,
                    #       "action_operation_type:", action_operation_type)
                ...
                if action_operation_type == current_operation_type: 
                    if action_operation_type == 1 or action_operation_type == 3:
                        ...
                        if len(able_pile_list) > 0:
                            ...
                            self.wrong_times += 1
                            self.total_wrong_time += 1
                            # step_reward = -3.0  # 或更大惩罚
                    elif action_operation_type == 2:
                        # print(">>>action_operation_type == 2")
        ...
        print(f"step_reward={step_reward}, terminated={terminated}, truncated={truncated}")
        # c_stack = f'{self.docker_game.options.block}{utils.to_double(container_bay)}{utils.to_double(container_row)}'
        scaled_reward = step_reward / 100.0

        return np.array(current_observation), scaled_reward, terminated, truncated, {}
    ...
    def get_valid_actions(self):
        """
        返回一个布尔列表，用于屏蔽非法动作（即操作类型与当前操作不一致的）
        """
        current_operation_status = self.observation_list[-1]
        current_operation_type = current_operation_status[
                        self.observation_map_item_map['OPERATION_TYPE'] - 2]
        valid_mask = [
            1 if action[-1] == current_operation_type else 0
    ...
    #     current_observation = self.observation_list[-1]

    #     if self.docker_game.options.with_predict:
    #         current_operation_type = current_observation[self.observation_map_item_map['OPERATION_TYPE']]
    #     else:
    #         current_operation_type = current_observation[self.observation_map_item_map['OPERATION_TYPE'] - 2]

    #     for idx, action in enumerate(self.action_map):
    #         action_op_type = action[-1]
    #         if action_op_type == current_operation_type:
    #             # 进一步可加入如堆位是否空等更精细限制
...
if __name__ == '__main__':
    ...
    for train_item in train_list:
        ...
        model = train_item['algorithm']['method']('MlpPolicy', vec_env, verbose=1, device='cuda', gamma=0.99,
                                                  ent_coef=0.01)
        model.learn(total_timesteps=100, log_interval=100)
